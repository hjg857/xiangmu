...
class Command(BaseCommand):
    ...
    def create_teacher_survey(self):
        ...
        template = SurveyTemplate.objects.create(
            ...
            description='''尊敬的老师：
            您好！为深入了解您所在学校教师数据素养的整体发展现状，以及各位教师对学校数据应用成效的主观评价，特设计《中小学校教师数据素养、数据资产意识及数据应用效果调查问卷》，诚邀您根据实际情况进行填写。此次调查严格遵循专业伦理与规范要求，确保您的个人隐私得到充分保护。您的回答无对错之分，请放心填写。衷心感谢您的支持与参与！
            在正式填写问卷之前，请仔细阅读以下相关概念：
            中小学校数据资产：指学校在日常教育教学、管理和服务过程中积累和产生的各类数据资源，这些数据具有一定的使用价值和管理价值，能够为学校的决策制定、教学优化、资源配置、学生发展以及整体管理等提供支持和依据。
            ''',
            is_active=True
        )
    ...
    def create_student_survey(self):
        ...
        template = SurveyTemplate.objects.create(
            ...
            description='''尊敬的同学：
您好！为深入了解您所在学校学生数据素养的整体发展现状，以及各位同学对学校数据应用成效的主观评价，特设计《中小学校学生数据素养调查问卷》，诚邀您根据实际情况进行填写。此次调查严格遵循专业伦理与规范要求，确保您的个人隐私得到充分保护。您的回答无对错之分，请放心填写。衷心感谢您的支持与参与！''',
            is_active=True
        )
